...
use crate::{m_chain::Chains, msg_chain::MsgChain};
use address::Address;
    ...
    T: Provider + Send + Sync + 'static,
{
    ...
    async fn select_messages_optimal(
        ...
        // TODO: Implement guess gas
        let min_gas = 1298450;
        // chains is a mapping of chain nodes along with any links that
        // gets create via the create_message_chain API
        // let mut chains = Chains::default();
        let (mut result, mut gas_limit) = self
            .select_priority_messages_new(&mut pending, &base_fee, &target_tipset)
            .await?;
        ...
        let mut chains = Chains::default();
        for (actor, mset) in pending.into_iter() {
            crate::m_chain::create_message_chains(
                &self.api,
                &actor,
                &mset,
                &base_fee,
        ...
        // Move this into gas_guess module
        pub(crate) const MIN_GAS: i64 = 1298450;
        pub(crate) const MAX_GAS: i64 = 1600271356;
        ...
        let mut partitions:Vec<Vec<crate::m_chain::NodeKey>> = vec![vec![]; MAX_BLOCKS];
        let mut i = 0;
        ...
        for i in 0..MAX_BLOCKS {
            for chain in &partitions[i] {
                chains.get_mut(*chain).unwrap().eff_perf = chains.get(*chain).unwrap().gas_perf * block_prob[i];
            }
            eff_chains += partitions[i].len();
        }

        // nullify the effective performance of chains that don't fit in any partition
        for chain_idx in eff_chains..chains.len() {
            let node = chains.get_mut_at(chain_idx).unwrap();
        ...
        for i in 0..chains.len() {
            // TODO: see if we can use the borrow as chain here.
            ...
            if chain_gas_limit <= gas_limit {
                ...
                chain_deps.iter().rev().for_each(|dep| {
                    let node = chains.get_mut(*dep).unwrap();
                    node.merged = true;
                ...
                if let Some(next_key) = chains[i].next {
                    ...
                    if next_node.eff_perf > 0.0 {
                        ...
                        let mut nnk = next_node.next;
                        while let Some(nnk_) = nnk {
                            let (nnode, prev_perfs) = chains.get_mut_with_prev_eff(nnk_);
                            if let Some(nnode) = nnode {
                                if nnode.eff_perf > 0.0 {
                                    nnode.set_eff_perf(prev_perfs);
                                }
                                nnk = nnode.next;
                            } else { 
                                break;
                ...
                dbg!("chain gas limit: {}", chain_gas_limit);
                dbg!("gas limit: {}", gas_limit);
            }
        ...
        if gas_limit >= MIN_GAS {
            ...
            crate::m_chain::shuffle_chains(&mut chains);

            for ch in 0..chains.len() {
                if gas_limit < MIN_GAS {
                    break;
                }
                
                // has it been merged or invalidated?
                if chains[ch].merged || !chains[ch].valid {
                    continue;
                }

                // is it negative?
                if chains[ch].gas_perf < 0.0 {
                    continue;
                }

                // compute the dependencies that must be merged and the gas limit including deps
                let mut chain_gas_limit = chains[ch].gas_limit;
                let mut dep_gas_limit = 0;
                let mut chain_deps = vec![];
                let mut cur_chain = chains[i].prev;
                while let Some(cur_chn) = cur_chain {
                ...
                if dep_gas_limit > gas_limit {
                    chains.invalidate(ch);
                    continue;
                }

                // do they fit as it? if it doesn't fit, trim to make it fit if possible
                if chain_gas_limit > gas_limit {
                    chains.trim_msgs_at(ch, gas_limit-dep_gas_limit, &base_fee);
                ...
                chains[ch].merged = true;
                result.extend(chains[i].msgs.clone());
                gas_limit -= chain_gas_limit;
                random_count += chains[ch].msgs.len();
            }
    ...
    async fn select_priority_messages_new(
        &self,
        pending: &mut Pending,
        base_fee: &BigInt,
        ts: &Tipset,
        ...
        Ok(merge_and_trim_new(&mut chains, result, base_fee, gas_limit, min_gas))
    }
}

/// Returns merged and trimmed messages with the gas limit
fn merge_and_trim_new(
    slot_chains: &mut Chains,
    ...
    // 2. Sort the chains
    slot_chains.sort();
    let first_chain_gas_perf = slot_chains[0].gas_perf;
    // NOTE: eplace this taken chain back into slot_chain with `place_key_vec_back`
    let mut chains = slot_chains.take_chain_vec();

    if !chains.is_empty() && first_chain_gas_perf < 0.0 {
        log::warn!("all priority messages in mpool have negative gas performance bestGasPerf: {}", first_chain_gas_perf);
    ...
    for (i, _) in chains.iter().enumerate() {
        let node = slot_chains.get_from_vec(&chains, i).unwrap();
    ...
    'tail_loop: while gas_limit >= min_gas && last < chain_len {
        ...
        slot_chains.trim_msgs_at(last, gas_limit, base_fee);
        
        // push down if it hasn't been invalidated
        let node = slot_chains.get_from_vec(&chains, last).unwrap();
        if node.valid {
            for i in last..chain_len - 1 {
                // slot_chains
                let cur_node = slot_chains.get_from_vec(&chains, i).unwrap();
                let next_node = slot_chains.get_from_vec(&chains, i + 1).unwrap();
        ...
        for (i, _) in chains.iter().skip(last).enumerate() {
            let chain = slot_chains.get_mut_from_vec(&chains, i).unwrap();
            if !chain.valid {
                continue;
            }
    ...
    slot_chains.place_key_vec_back(chains);
    (result, gas_limit)
}
...
mod test_selection {
    ...
    async fn test_optimal_msg_selection2() {
        ...
        // We select with a low ticket quality; the chain dep        // for i in &chains.key_vec {
        //     dbg!(&i);
        // }enent merging algorithm should pick
